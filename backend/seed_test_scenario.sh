...
# - Owner: admin1@fligolf.com (participant 1/6)
# - 4 approved participants: user1-4 (participants 2-5/6)
# - 1 pending join request from user5 (waiting for approval)
#
# When owner approves user5's request:
# - League becomes 6/6
# - Hook triggers: status -> ready, draft positions assigned, fantasy_tournaments created
#
# Login as: admin1@fligolf.com / MADcap(123)
...
echo "  1/6: admin1 (owner)"

# Add participants 2-5 (user1-4) - these are already approved
for i in 1 2 3 4; do
...
# Create pending join request from user5
echo ""
echo "Creating pending join request..."
user5_id=$(curl -s "$PB_URL/api/collections/user_profiles/records?filter=(display_name='user5')" | grep -o '"user_id":"[^"]*"' | head -1 | cut -d'"' -f4)
...
echo "  Pending request from user5 -> $request_id"
...
echo "  4. user3"
echo "  5. user4"
echo "  6. [PENDING] user5 - waiting for approval"
echo ""
echo "Login as owner: admin1@fligolf.com / MADcap(123)"
echo ""
echo "To approve user5 and trigger 6/6:"
echo "  1. Go to PocketBase Admin -> join_requests"
echo "  2. Find user5's request, change status to 'approved'"
echo "  3. Then create participant for user5 in fantasy_participants"
echo ""
